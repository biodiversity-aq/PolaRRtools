...
#
#   Script content:
#   "TOOLS FOR SUBMITTING DATA TO ENA"
#
#==============================================================
...
#--------------------------------------------------------------
# Renaming sequence files in a folder
#--------------------------------------------------------------
FileNames.to.Table <- function(file.dir=NULL, paired=TRUE, seq.file.extension=".fastq.gz",
  ...
  #' @description makes a table with the all the names of sequence files in a folder. The aim is to save this as a CSV file, fill in the column with new name, and then use the rename.sequenceFiles() function to rename the files.
  ...
  #' @details fastq files from sequencing facilities often come with long and complex file names that were automatically generated by the sequencer machine, and no longer resemble the original name of the sample. These tools help to get the original file names, and easily convert them back into the original file names.
  ...
  if(paired){
    ...
    if(length(intersect(names_core_fw, names_core_rv))==length(names_core_fw)){
      ...
      names_out <- data.frame(NewName="", OldName=names(names_fw), 
      ...
      if(length(names_rv_extra)>0){
        fw_out <- data.frame(NewName="", OldName=names_rv_extra, 
                             ForwardName="",
